...
{
    private readonly bool _hasAdministrativeRights;
    ...
    {
        _service = service;
        _hasAdministrativeRights = Program.HasAdministrativeRight();
        ...
        {
            Trace.WriteLine("Event Log does not exist");
        }
    }
    ...
    {
        var path = Program.GetConfigurationPath();
        var f = new FormConfigureOverview(path);
        if(f.ShowDialog(this) == DialogResult.OK)
        {
            Trace.WriteLine($"Saved to {path}");
            UpdateServiceStatus();
            if(_hasAdministrativeRights && _service?.Status == ServiceControllerStatus.Running)
            {
